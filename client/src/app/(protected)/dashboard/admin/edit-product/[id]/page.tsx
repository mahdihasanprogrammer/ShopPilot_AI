...
export default function EditProductPage({
  ...
  useEffect(() => {
    ...
    } else if (!sessionLoading && session && !isAdmin) {
  ...
  if (loading || sessionLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center min-h-[50vh] gap-3">
        ...
        <p className="text-xs font-semibold text-text-neutral/50">Loading product details…</p>
      </div>
    );
  }
  ...
  return (
    <main className="flex-1 px-6 py-12 sm:px-8 lg:px-12 max-w-7xl mx-auto w-full space-y-8">
      ...
      <nav className="flex items-center gap-1.5 text-xs font-semibold text-text-neutral/50" aria-label="Breadcrumb">
        ...
        <FiChevronRight className="h-3 w-3" />
        <span className="text-text-neutral font-bold">Edit Product</span>
      </nav>

      {/* Header */}
      <div className="border-b border-bg-secondary pb-6">
        <h1 className="text-2xl font-extrabold tracking-tight text-text-neutral">Edit Product</h1>
        <p className="text-xs text-text-neutral/50 font-medium mt-0.5">Modify information for "{form.title || 'Product'}"</p>
      </div>
          ...
          {FIELDS.map((field) => {
            ...
            return (
              ...
              >
                ...
                {err && (
                  <p id={`${field.name}-error`} role="alert" className="mt-1.5 flex items-center gap-1 text-xs font-semibold text-red-500">
